...
=============================================================================*/
#ifndef SPRIG_BRAIN_F_CK_ITERATION_MULTI_ITERATION_HPP
#define SPRIG_BRAIN_F_CK_ITERATION_MULTI_ITERATION_HPP
...
#include <sprig/brain/f_ck/literals/multi_literals.hpp>
...
namespace sprig {
	namespace brain {
		namespace f_ck {
			...
			// bf_multi_iteration
			//
			//	COMMENT: 基本コマンドに対応する文字列セットを扱う走査クラス
			//
			template<
			...
			public:
				typedef typename select_default<Literals, bf_multi_literals>::type literals;
			private:
				template<size_type N>
				size_type match_next(position_interface<types>& pos) {
					int index = 0;
					BOOST_FOREACH(
						typename boost::range_reference<typename literals::type const>::type e,
						literals::template get<N>()
						)
					{
				...
				template<size_type N>
				size_type match_prior(position_interface<types>& pos) {
					int index = 0;
					BOOST_FOREACH(
						typename boost::range_reference<typename literals::type const>::type e,
						literals::template get<N>())
				...
				void advance_next(position_interface<types>& pos) {
					...
					std::advance(i, static_cast<diff_type>(match_next<N>(pos)));
				}
				template<size_type N>
				void advance_prior(position_interface<types>& pos) {
					position_iterator<types> i = pos;
					std::advance(i, -static_cast<diff_type>(match_prior<N>(pos)));
				}
			public:
				virtual std::auto_ptr<iteration_interface> clone() {
					std::auto_ptr<iteration_interface> p(new bf_multi_iteration(*this));
					return p;
				}
...
#endif	// #ifndef SPRIG_BRAIN_F_CK_ITERATION_MULTI_ITERATION_HPP
