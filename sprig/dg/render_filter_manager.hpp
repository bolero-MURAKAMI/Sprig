#ifndef SPRIG_DG_RENDER_FILTER_MANAGER_HPP
#define SPRIG_DG_RENDER_FILTER_MANAGER_HPP
...
#include <sprig/dg/render_filter.hpp>

namespace sprig {
	namespace dg {
		//
		// render_filter_manager
		//
		template<
			typename Key = std::basic_string<char_type>,
			typename Priority = double
		>
		class render_filter_manager
			: public render_filter
		{
		private:
			typedef render_filter_manager this_type;
		public:
			typedef render_filter filter_type;
		...
		public:
			...
			HRESULT reinitialize(sprig::call_traits<device_type>::param_type device) {
				SPRIG_DG_SECTION(TEXT("render_filter_manager::reinitialize"));
			...
			result_type apply(locked_data_type const& data) const {
				result_type result(D3D_OK, data);
			...
			insert_result_type insert(
				...
				typename sprig::call_traits<instance_type>::param_type render
				)
			{
				return priority_map_.insert(value_type(key, priority, render));
			}
		...
		SPRIG_DG_SPECIALIZED_VALUE_INFO_TEMPLATE_OVERLOADS(0, 3, render_filter_manager);
		template<typename Logger>
		SPRIG_INLINE void value_info(render_filter_manager<> const& value, Logger& logger) {
			logger.section_line(TEXT("type"), typeid(value).name());
			value.members_value_info(logger);
		}
		template<typename Key, typename Logger>
		SPRIG_INLINE void value_info(render_filter_manager<Key> const& value, Logger& logger) {
			logger.section_line(TEXT("type"), typeid(value).name());
			value.members_value_info(logger);
		}
		template<typename Key, typename Priority, typename Logger>
		SPRIG_INLINE void value_info(render_filter_manager<Key, Priority> const& value, Logger& logger) {
			logger.section_line(TEXT("type"), typeid(value).name());
			value.members_value_info(logger);
		}
	}	// namespace dg
